...
public class Main {
    ...
    private static void scheduleRace(Scanner sc) {
        ...
        while (true) {
            try {
                System.out.print("Enter number of laps (0-80): ");
                String input = sc.nextLine().trim();
                ...
                System.out.println("Invalid input. Please enter a whole number between 0 and 80.");
            }
        }
    ...
    private static void generateSponsorshipOffers() {
        ...
        // Descending points, then descending wins
        ...
        // Ascending points, then ascending wins
        ...
        if (!highTeams.isEmpty()) {
            for (int i = 0; i < highSponsors.size(); i++) {
                ...
                flag1 = SponsorshipOfferDAO.generateSponsorship(team.teamId, sp.name, sp.industry, sp.contractValue, sp.contractDurationMonths);
            }
        }
        if (!lowTeams.isEmpty()) {
            for (int i = 0; i < lowSponsors.size(); i++) {
                ...
                flag2 = SponsorshipOfferDAO.generateSponsorship(team.teamId, sp.name, sp.industry, sp.contractValue, sp.contractDurationMonths);
            }
        }
        ...
        if(flag1 || flag2) {
            ...
            System.out.println("NO Sponsorship offers generated.");
        }
    }
    ...
    private static void createAccount(Scanner sc) {
        ...
        while (true) {
            try {
                System.out.print("Enter Full Name (Firstname and Surname) or 'back' to cancel: ");
            ...
            switch (choice) {
                ...
                case 9:
                    ...
                    if (searchedUser == null || searchedUser.teamId == 0) {
                        ...
                        displayTeamStatistics(searchedUser.userId);
                    }
                    break;
                case 10:
    ...
    private static void buyCar(Scanner sc, User user) {
        ...
        for (Car car : cars) {
            System.out.printf("│ %-5d │ %-25s │ %-11d │ %-9d │ %,10d │%n",
                    car.getCarId(), car.getCarName(), car.enginePower, car.maxSpeed, car.getPrice());
        }
        ...
        if (team.getBudget() < car.getPrice()) {
            System.out.println("⚠ Not enough budget to buy this car. Your budget: " + team.getBudget() + ", Car price: " + car.getPrice());
            return;
        }

        if (CarDAO.assignCarToTeam(carId, team.getTeamId(), car.getPrice())) {
            System.out.printf("✅ Car '%s' purchased successfully. Remaining budget: %,d%n", car.getCarName(), team.getBudget() - car.getPrice());
        } else {
            System.out.println("⚠ Failed to purchase car. Please try again later.");
        }
    }

    private static void buyDriver(Scanner sc, User user) {
        ...
        System.out.println("┌───────┬───────────────────────────┬─────────────┬───────────────┐");
        System.out.printf("│ %-5s │ %-25s │ %-11s │ %-13s │%n", "ID", "Driver Name", "Skill Level", "Price");
        System.out.println("├───────┼───────────────────────────┼─────────────┼───────────────┤");

        for (Driver driver : drivers) {
            System.out.printf("│ %-5d │ %-25s │ %-11d │ %,13d │%n",
                    driver.getDriverId(), driver.getDriverName(), driver.skillLevel, driver.getRentalPrice());
        }

        System.out.println("└───────┴───────────────────────────┴─────────────┴───────────────┘");

        System.out.print("Enter Driver ID to Purchase (or 0 to cancel): ");
        String input = sc.nextLine();
        int driverId;
        try {
        ...
        Team team = TeamDAO.getTeamById(user.teamId);
        if (team == null) {
            System.out.println("Team not found. Cannot proceed.");
            return;
        }
        if (team.getBudget() < driver.getRentalPrice()) {
            System.out.println("⚠ Not enough budget to purchase this driver. Your budget: " + team.getBudget() + ", purchase price: " + driver.getRentalPrice());
            return;
        }

        if (DriverDAO.assignDriverToTeam(driverId, team.getTeamId(), driver.getRentalPrice())) {
            System.out.printf("✅ Driver '%s' rented successfully. Remaining budget: %,d%n", driver.getDriverName(), team.getBudget() - driver.getRentalPrice());
        } else {
            System.out.println("⚠ Failed to purchase driver. Please try again later.");
        }
    }
    ...
    private static boolean viewUpcomingRaces() {
        ...
        BinarySearchTree<LocalDateTime, Race> bst = new BinarySearchTree<>();
        for (Race race : races) {
        ...
        System.out.println("--------------------------------------------------------");
        for (Race race : bst.inOrderValues()) {
    ...
    private static void registerTeamForRace(Scanner sc, User user) {
        ...
        for (Car car : teamCars) {
            System.out.printf("%d: %s%n", car.getCarId(), car.getCarName());
        }
        int carId = 0;
        while (true) {
            ...
            try {
                ...
                boolean validCar = teamCars.stream().anyMatch(c -> c.getCarId() == finalCarId);
                if (validCar) break;
        ...
        for (Driver driver : teamDrivers) {
            System.out.printf("%d: %s%n", driver.getDriverId(), driver.getDriverName());
        }
        int driverId = 0;
        while (true) {
            ...
            try {
                ...
                boolean validDriver = teamDrivers.stream().anyMatch(d -> d.getDriverId() == finalDriverId);
    ...
    public static void listCarIdsAndNames(List<Car> cars) {
        ...
        System.out.println("├" + "─".repeat(40) + "┤");
        for (Car car : cars) {
            System.out.printf("│ %-5d │ %-30s │\n", car.getCarId(), car.getCarName());
        }
        System.out.println("└" + "─".repeat(40) + "┘");

    }

    private static void displayTeamStatistics(int userId) {
        map.loadUserTeamMap();
        Team userTeam = map.getTeamByUserId(userId);
        ...
        if (drivers.isEmpty()) {
            ...
            for (Driver d : drivers) {
                System.out.printf("   • %s (Skill Level: %d, Rental Price: $%,d)\n",
                        d.getDriverName(), d.skillLevel, d.getRentalPrice());
            }
        }
